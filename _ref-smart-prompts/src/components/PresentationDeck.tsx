...
import { GridOverviewDeck } from "./GridOverviewDeck";
import { HelpOverlay } from "./HelpOverlay";
import { PresenterViewDeck } from "./PresenterViewDeck";
...
export function PresentationDeck({ slides, storageKey, deckTitle }: Props) {
  ...
  const next = () => {
    const maxStep = slides[index]?.steps ?? 0;
    if (step < maxStep) {
      setStep((s) => s + 1);
      return;
    }
